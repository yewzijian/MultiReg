...
from common.lie.numpy.so3 import SO3
...
class SE3(LieGroupBase):
    ...
    @staticmethod
    def rand(size: int = None, rot_mag=np.pi, trans_mag=1.0) -> 'SE3':
        """Generates a random rotation

        Note that the rotation is only uniformly random if rot_mag is np.pi.
        """
        rot = SO3.rand(size, rot_mag)
        trans = np.random.randn(3, 1) * trans_mag if size is None else \
            np.random.randn(size, 3, 1) * trans_mag
